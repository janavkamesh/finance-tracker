...
### Insights year-switch — anticipatory prefetch for zero-latency navigation
Switching years in the Insights page caused a 1-2 second delay on first click because the server component had to run a fresh Supabase query for the new year. Fixed by adding a `useEffect` in `components/reports/year-selector.tsx` that calls `router.prefetch()` for the two prior years (and next year when not already on the current year) immediately after the current year's UI mounts. Next.js populates its RSC payload cache in the background with no loading indicator; when the user clicks the arrow the cached payload resolves synchronously, making every year transition instant regardless of whether it has been visited before.

### Pixel-perfect alignment & consistency pass — Home, Transactions, Goals

**Flush-left alignment fix (Home + Transactions)**: Transaction rows previously had a visual stagger between the Description text and the Category pill because the pill was either leading with an icon inside it or was ordered after the date, breaking the shared left-edge contract. Two changes enforce identical left-alignment: (1) `animated-transaction-list.tsx` (Home) now renders a standalone circular category icon container (`h-9 w-9 rounded-lg`) as the first element in each row — the same pattern used by the Transactions master list — so both Description and Category pill share the same `div.min-w-0` origin point and are perfectly flush. The category pill is now text-only (icon removed from inside the pill, since the standalone icon handles that role). (2) `transaction-manager.tsx` (Transactions) reorders the meta row so the Category pill is always the first element, followed by the date — the pill's outer left edge now aligns exactly with the Description text above it, and the date flows naturally to the right.

**Remove duplicate payment method (Transactions)**: The Payment Method label (e.g., "UPI") was rendered twice per row in the master list — once in the meta row (correct) and again right-aligned below the amount (incorrect). The redundant `paymentLabel` block under the amount column in `transaction-manager.tsx` has been removed. The amount column now shows only the currency figure. The unused `paymentLabel` variable in the row render loop was also cleaned up.

**Universal component porting — Recurring Bills rows (Goals)**: Rows in the Recurring Bills & Subscriptions card on `goals/page.tsx` previously used a plain `h-2 w-2` dot as the only visual identifier. They now match the transaction design system: each row opens with a `h-9 w-9 rounded-lg` category icon container using `getCategoryIcon` (same icon lookup used by both transaction lists), and the category name is wrapped in the standard rounded-md pill badge with `px-1.5 py-0.5` padding and colour-aware styling (custom-category colour or neutral gray fallback). The Supabase query was updated to fetch `color, icon, user_id` from the joined `categories` table to support colour-aware icon and pill styling.

### Dark mode chart grid lines — theme-adaptive `--chart-grid` token
All Recharts charts (`MonthlyBarChart`, `AreaTrendChart`) used a hardcoded `stroke="#f0f0f0"` on `CartesianGrid`. In light mode this is an imperceptibly soft line (94% white) — premium. In dark mode it rendered as near-white against the `#111620` Obsidian Vault background — harsh and cheap. Root cause: `#f0f0f0` has no dark-mode awareness and SVG stroke attributes cannot be targeted by Tailwind dark-mode variants or CSS class overrides.

Fix: introduced a single semantic design token `--chart-grid` into `globals.css` alongside the existing Obsidian Vault variable definitions — `rgba(0,0,0,0.06)` in light mode (matching the previous feel exactly) and `rgba(255,255,255,0.09)` in dark mode (barely-there white glow: present enough to structure the chart space, never harsh). Both `CartesianGrid` components now use `stroke="var(--chart-grid)"`. The `Tooltip` `contentStyle` border was simultaneously upgraded from `#e5e7eb` to `border: "1px solid var(--chart-grid)"` with `background: "var(--bg-elevated)"` and `color: "var(--text-primary)"` so the tooltip card is also fully theme-aware without relying on the existing CSS `!important` patch.
...
### Bug fix — duplicate transaction row on add (optimistic + server)
When adding a transaction, two rows were briefly visible simultaneously: the optimistic row (instant) and a real row after 1–2 s (server revalidation). Root cause: Next.js RSC re-renders create a new `initialTransactions` array reference on every server render — even when the IDs are identical — triggering the `useEffect` in `TransactionManager` and wiping the optimistic entry before the real transaction arrived; the sequence appeared as two separate "creations." Fix: added `prevInitRef` to compare IDs between consecutive `initialTransactions` values; the effect is now skipped when IDs are identical (reference-only change), so the optimistic row stays visible until the genuine data change (new UUID from the server) arrives and replaces it cleanly.

### Bug fix — CategoryPicker requiring 3+ clicks to select a category
Selecting a category inside the picker was unreliable — it took 1–4 clicks before the selection registered. Root cause: the panel was rendered via `createPortal` to `document.body`, placing it outside the Base UI `Dialog.Popup`'s DOM tree. Base UI's focus trap runs in the native event **capture phase** at the document root; when focus moved into the portal (e.g. the auto-focused search input), the trap immediately pulled focus back into the dialog, interrupting the click sequence before `onClick` could fire. Previous `nativeEvent.stopImmediatePropagation()` hacks fired in the React **bubble phase** — too late to intercept Base UI's capture-phase listener. Fix: removed `createPortal` entirely. The backdrop and panel are now rendered as `position: fixed` elements **inside** the dialog's DOM tree. Fixed-positioned elements are not clipped by ancestor overflow, so the visual result is identical, but Base UI's focus trap now sees all picker interactions as "inside" the dialog and stops interfering. The `nativeEvent.stopImmediatePropagation()` calls and the `mounted` state were also removed as they are no longer needed.

### Build fix — remove middleware.ts conflict with proxy.ts (Vercel)
Vercel rejected the build with: `Both middleware file "./middleware.ts" and proxy file "./proxy.ts" are detected. Please use "./proxy.ts" only.` Root cause: the NextAuth integration (added separately) introduced a root-level `middleware.ts` alongside the existing `proxy.ts` that Vercel requires for edge routing. Both files called `updateSession` from `lib/supabase/middleware` — identical logic, different filenames. Fix: merged the `api/auth` path exclusion from `middleware.ts`'s matcher into `proxy.ts` (so NextAuth endpoints remain unintercepted by the Supabase session middleware), then deleted `middleware.ts`. All auth and session-refresh logic is now consolidated in `proxy.ts`.

### Bug fix — Weekly trend chart: real date-based bucketing replaces fake weight distribution
The "Weekly trend" bar chart was distributing amounts using hardcoded weight arrays (`INCOME_WEIGHTS`, `EXPENSE_WEIGHTS`) applied to the current month's aggregate total — e.g. a single ₹10,000 expense produced four non-zero bars instead of one. Root cause: `buildWeeklyData` in `trend-chart-card.tsx` received only the pre-aggregated `MonthlyData[]` (one total per month) and had no access to individual transaction dates, so it could not place amounts into their correct week.

Fix: `TrendChartCard` now accepts a second prop `transactions: Transaction[]` (current-month raw transactions passed from `dashboard/page.tsx` as `currentMonthTxns`). `buildWeeklyData` iterates every transaction, reads its `date` field, extracts the day-of-month, and maps it into one of four strict buckets (days 1–7 → Week 1, 8–14 → Week 2, 15–21 → Week 3, 22+ → Week 4). Income and expense amounts are summed independently per bucket. Weeks with no transactions correctly show ₹0. The fake weight constants were removed entirely.
.
