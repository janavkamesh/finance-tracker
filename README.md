...
### Bug fix — CategoryPicker delete / actions closing the parent TransactionDialog
Clicking Delete (or any action) inside the CategoryPicker panel was dismissing the parent Transaction dialog. Root cause: the CategoryPicker panel renders as a `createPortal` onto `document.body`, outside the Base UI `Dialog.Popup`'s DOM tree. Base UI attaches a native `pointerdown` listener on `document` to detect outside clicks; since the portal is not a descendant of the dialog, every pointer-down inside the portal looked like an outside click and closed the dialog. Fix: added `onPointerDown` handlers on both the portal backdrop div and the inner panel div that call `e.nativeEvent.stopImmediatePropagation()`, intercepting the native pointer event before it reaches Base UI's document-level listener. The picker's own close-on-backdrop-click (`onMouseDown={handleClose}`) is unaffected.
